...
class TelemetryPacker:
    ...
    def pack_tm_frame(cls):
        ...
        # Light sensor X+
        cls.PACKET[137:139] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_XP)
        # Light sensor X-
        cls.PACKET[139:141] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_XM)
        # Light sensor Y+
        cls.PACKET[141:143] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_YP)
        # Light sensor Y-
        cls.PACKET[143:145] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_YM)
        # Light sensor Z+ 1
        cls.PACKET[145:147] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_ZP1)
        # Light sensor Z+ 2
        cls.PACKET[147:149] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_ZP2)
        # Light sensor Z+ 3
        cls.PACKET[149:151] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_ZP3)
        # Light sensor Z+ 4
        cls.PACKET[151:153] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_ZP4)
        # Light sensor Z-
        cls.PACKET[153:155] = pack_unsigned_short_int(adcs_data, ADCS_IDX.LIGHT_SENSOR_ZM)

        # XP coil status
        cls.PACKET[155] = adcs_data[ADCS_IDX.XP_COIL_STATUS] & 0xFF
        # XM coil status
        cls.PACKET[156] = adcs_data[ADCS_IDX.XM_COIL_STATUS] & 0xFF
        # YP coil status
        cls.PACKET[157] = adcs_data[ADCS_IDX.YP_COIL_STATUS] & 0xFF
        # YM coil status
        cls.PACKET[158] = adcs_data[ADCS_IDX.YM_COIL_STATUS] & 0xFF
        # ZP coil status
        cls.PACKET[159] = adcs_data[ADCS_IDX.ZP_COIL_STATUS] & 0xFF
        # ZM coil status
        cls.PACKET[160] = adcs_data[ADCS_IDX.ZM_COIL_STATUS] & 0xFF

        # Coarse attitude QW
        cls.PACKET[161:165] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.COARSE_ATTITUDE_QW])
        # Coarse attitude QX
        cls.PACKET[165:169] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.COARSE_ATTITUDE_QX])
        # Coarse attitude QY
        cls.PACKET[169:173] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.COARSE_ATTITUDE_QY])
        # Coarse attitude QZ
        cls.PACKET[173:177] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.COARSE_ATTITUDE_QZ])

        # Star tracker status
        cls.PACKET[177] = adcs_data[ADCS_IDX.STAR_TRACKER_STATUS] & 0xFF

        # Star tracker attitude QW
        cls.PACKET[178:182] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.STAR_TRACKER_ATTITUDE_QW])
        # Star tracker attitude QX
        cls.PACKET[182:186] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.STAR_TRACKER_ATTITUDE_QX])
        # Star tracker attitude QY
        cls.PACKET[186:190] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.STAR_TRACKER_ATTITUDE_QY])
        # Star tracker attitude QZ
        cls.PACKET[190:194] = convert_float_to_fixed_point_hp(adcs_data[ADCS_IDX.STAR_TRACKER_ATTITUDE_QZ])

        ############ GPS fields ############
        gps_data = DH.get_latest_data("gps")

        # message ID
        cls.PACKET[194] = gps_data[GPS_IDX.GPS_MESSAGE_ID] & 0xFF
        # fix mode
        cls.PACKET[195] = gps_data[GPS_IDX.GPS_FIX_MODE] & 0xFF
        # number of SV
        cls.PACKET[196] = gps_data[GPS_IDX.GPS_NUMBER_OF_SV] & 0xFF
        # GNSS week
        cls.PACKET[197:199] = pack_unsigned_short_int(gps_data, GPS_IDX.GPS_GNSS_WEEK)
        # GNSS TOW
        cls.PACKET[199:203] = pack_unsigned_long_int(gps_data, GPS_IDX.GPS_GNSS_TOW)
        # latitude
        cls.PACKET[203:207] = pack_signed_long_int(gps_data, GPS_IDX.GPS_LATITUDE)
        # longitude
        cls.PACKET[207:211] = pack_signed_long_int(gps_data, GPS_IDX.GPS_LONGITUDE)
        # ellipsoid altitude
        cls.PACKET[211:215] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ELLIPSOID_ALT)
        # mean sea level altitude
        cls.PACKET[215:219] = pack_signed_long_int(gps_data, GPS_IDX.GPS_MEAN_SEA_LVL_ALT)

        # ECEF X
        cls.PACKET[219:223] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_X)
        # ECEF Y
        cls.PACKET[223:227] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_Y)
        # ECEF Z
        cls.PACKET[227:231] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_Z)
        # ECEF VX
        cls.PACKET[231:235] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_VX)
        # ECEF VY
        cls.PACKET[235:239] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_VY)
        # ECEF VZ
        cls.PACKET[239:243] = pack_signed_long_int(gps_data, GPS_IDX.GPS_ECEF_VZ)

        ############ Thermal fields ############
        thermal_data = DH.get_latest_data("thermal")

        # IMU temperature
        cls.PACKET[244:246] = pack_unsigned_short_int(thermal_data, THERMAL_IDX.IMU_TEMPERATURE)
        # CPU temperature
        cls.PACKET[246:248] = pack_unsigned_short_int(thermal_data, THERMAL_IDX.CPU_TEMPERATURE)
        # Battery temperature
        cls.PACKET[248:250] = pack_unsigned_short_int(thermal_data, THERMAL_IDX.BATTERY_PACK_TEMPERATURE)
