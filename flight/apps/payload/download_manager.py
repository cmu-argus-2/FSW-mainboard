...
class DownloadManager:
    ...
    def note_fragment_received(self, fragment):
        ...
        if fragment.tid != self.current_tid:
            logger.warning(
                f"[DOWNLOAD_MGR] Fragment tid={fragment.tid} does not match current transaction tid={self.current_tid}."
            )
            return
    ...
    def process_batch(self, uart_reader_callback):
        ...
        while TPM.time() - last_received_packet < self.TIMEOUT_BETWEEN_PACKETS:
            ...
            if self.current_batch_received_count >= target_batch_count:
                logger.info(
                    ...
                    f"for tid={self.current_tid}, seq_offset={self.current_batch_offset}. "
                    f"Ending listen early."
                )
                break
        ...
        logger.info(
            ...
            f"seq_offset={self.current_batch_offset}, width={width}, "
            f"bitmap=0x{bitmap:016X}, missing_frags={trans._missing_fragments_count}"
        )
    ...
    def advance_to_next_file(self):
        ...
        """
        logger.info(f"[DOWNLOAD_MGR] Advancing from tid={self.current_tid}. Queue size: {len(self.transaction_queue)}")
            ...
            status.update(
                {
                    ...
                    "received_packets": (
                        (self.current_transaction.number_of_packets - missing_count)
                        if self.current_transaction.number_of_packets
                        else 0
                    ),
