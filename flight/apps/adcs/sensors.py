...
def current_mode(current_mode) -> int:
    ...
    if current_mode == Modes.TUMBLING:
        if np.linalg.norm(omega) <= Modes.TUMBLING_LO:
            return Modes.STABLE
        else:
            return Modes.TUMBLING

    elif current_mode == Modes.STABLE:
        momentum_error = np.linalg.norm(ControllerConst.MOMENTUM_TARGET - np.dot(PhysicalConst.INERTIA_MAT, omega))

        if np.linalg.norm(omega) >= Modes.TUMBLING_HI:
            return Modes.TUMBLING
        ...
        sun_error = np.linalg.norm(
            sun_pos_body - np.dot(PhysicalConst.INERTIA_MAT, omega) / np.linalg.norm(ControllerConst.MOMENTUM_TARGET)
        )

        if momentum_error >= Modes.STABLE_TOL_HI:
            return Modes.STABLE

        elif sun_error <= Modes.SUN_POINTED_TOL:
