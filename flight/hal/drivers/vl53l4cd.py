...
* Author(s): Carter Nelson, Perrin Tong
...
class VL53L4CD:
    ...
    def timing_budget(self, val):
        ...
        if inter_meas != 0 and val > inter_meas:
            raise ValueError(f"Timing budget can not be greater than inter-measurement period ({inter_meas})")
    ...
    def inter_measurement(self, val):
        ...
        if val != 0 and val < timing_bud:
            raise ValueError(f"Inter-measurement period can not be less than timing budget ({timing_bud})")
